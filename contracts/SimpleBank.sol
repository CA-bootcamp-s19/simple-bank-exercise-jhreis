...
contract SimpleBank {
    ...
    function deposit() payable public returns (uint) {
        ...
        uint newBalance = curBalance + msg.value;
    ...
    function withdraw(uint withdrawAmount) public returns (uint) {
        ...
        // How much the user currently has in the bank
        uint available = balances[msg.sender];

        // User can never withdraw more than they have
        require(available >= withdrawAmount);

        // What the new balance will be after withdrawing funds
        uint newBalance = available - withdrawAmount;

        // Updates to the new balance, _before_ attempting the send!
        balances[msg.sender] = newBalance;
